...
{{--                                    </form>--}}
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                                    ...
                                                    <div class="d-flex justify-content-center mt-5">
                                                        <div class="card-body bg-light rounded">
                                                            <input type="file" class="form-control dropify" id="image"
                                                                   ...
                                                                   data-default-file="{{(isset($brand) && !empty($brand->image) && File::exists('uploads/brands/'.$brand->image)) ? asset('uploads/brands/'.$brand->image):asset('uploads/brands/default.png') }}"
                                            ...
                                            <div class="col-md-12 mb-3">
                                                <label class="form-label font-weight-bold text-muted text-uppercase">Brand Name<span class="text-danger">*</span></label>
                    ...
                    <div class="modal fade" id="editBrandModal" tabindex="-1" role="dialog" aria-labelledby="EditBrandTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="EditBrandTitle">Edit Brand</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                        ...
                                        <div class="col-md-12 mb-3">
                                            <label class="form-label font-weight-bold text-muted text-uppercase">Brand Name<span class="text-danger">*</span></label>
                                            <input type="text" class="form-control" id="edit_name" name="edit_name" placeholder="Enter Brand Name">
                                        </div>
                                    ...
                                    <div class="table table-responsive">
                                        <table class="table" id="datatable">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col"><label class="text-muted mb-0">Brand Name</label></th>
...
<script>
                $(document).ready(function () {
                    $('#datatable').DataTable({
                        ...
                        processing: true,
                        serverSide: true,
                        ajax: "{{ url('admin/inventory/brands/') }}",
                        columns: [
    ...
    function changeStatus(id) {
        if (id) {
            $.ajax({
                url: "{{ url('admin/inventory/brands/updateStatus') }}",
                data: {
                    id: id
                },
                type: "GET",
...
<script>
    ...
    $(document).on("click", ".add_brand", function (event) {
        ...
        if($('#name').val()=="") {
            swal("Failure!", "Please enter brand name.", "error");
            return false;
        }
        $.ajax({
            url: "{{ url('admin/inventory/brands/create') }}",
            data: postData,
            cache: false,
            contentType: false,
            processData: false,
            ...
            success: function (data) {
                if (data == "error") {
                    swal("Failure!", "Brand Name already available.", "error");
                } else {
...
<script>
    ...
    $('body').on('click', '.editBrand', function (event) {
        var id = $(this).data('id');
        var url = "{{ url('admin/inventory/brands/update') }}" + '/' + id;
        $.get(url, function (data) {
            $('#editBrandModal').modal('show');
            $('#edit_name').val(data.data.name);
            $('#id').val(data.data.id);
            var nameImage = "{{ asset('uploads/brands') }}" + '/' + data.data.image;
...
<script>
    ...
    $(document).on("click", ".update_brand", function (event) {
        ...
        var id = $('#id').val();
        $.ajax({
            url: "{{ url('admin/inventory/brands/update') }}" + '/' + id,
            data: postData,
            cache: false,
            contentType: false,
            processData: false,
    ...
    <script>
        ...
        $(document).on("click", ".change-status", function () {
            ...
            if (id) {
                $.ajax({
                    url: "{{ url('admin/inventory/brands/change-status-brands/') }}",
                    data: {
                        id : id
                    },
                    type: "GET",
